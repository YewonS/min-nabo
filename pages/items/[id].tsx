      ...
      <div className="px-4 py-4">
        <div className="mb-8">
          ...
          <div className="relative h-96">
            <Image
              src={`https://imagedelivery.net/Qb7PA1G1cVeln13aN4KZiQ/${data?.item?.imageURL}/public`}
          ...
          <div className="flex cursor-pointer py-3 border-t border-b items-center space-x-3">
            { data?.item?.user?.avatar ? 
              <img
                src={`https://imagedelivery.net/Qb7PA1G1cVeln13aN4KZiQ/${data?.item?.user?.avatar}/avatar`}
                className="w-12 h-12 rounded-full bg-slate-300"
              />
              : <div className="w-12 h-12 rounded-full bg-slate-300" />
            }
            <div>
              <p className="text-sm font-medium text-gray-600">{data?.item?.user?.name}</p>
          ...
          <div className="mt-8">
            ...
            <div className="flex items-center justify-between space-x-2">
              ...
              <button 
                onClick={onFavClick}
                className={
                  joinClassNames("p-3 rounded-md flex items-center justify-center", data?.isLiked ? "text-red-500 hover:bg-gray-200" : "text-gray-500 hover:bg-gray-200")
                }
              >
                <svg
                  className="h-6 w-6"
                  aria-hidden="true"
                >
                  <FontAwesomeIcon icon={data?.isLiked ? faHeartFull : faHeart } />
                </svg>
              </button>
            </div>
          </div>
